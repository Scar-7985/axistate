...
const PhotosVideo = () => {
    ...
    const [currentItem, setCurrentItem] = useState({ video: "", thumbnail: null });

    const handleCurrentChange = (e, type) => {
        const { value, files } = e.target;

        if (type === "video") {
            setCurrentItem(prev => ({ ...prev, video: value }));
        } else if (type === "thumbnail") {
            setCurrentItem(prev => ({ ...prev, thumbnail: files[0] }));
        }
    };


    const handleAdd = () => {
        if (!currentItem.video || !currentItem.thumbnail) {
            swalMsg("error", "Add all required fields", 2000);

            return;
        }


        setVideoData([...videoData, currentItem]);
        setCurrentItem({ video: "", thumbnail: null });
        const closeModal = document.querySelector("#dismisModal");
        closeModal.click();
    };
    ...
    const handleSubmit = () => {
        if (isLoading) {
            return;
        } else if (files.length === 0 || videoData.length === 0) {
            swalMsg("error", "Atleast one video & one photo is required", 2000);
        ...
        videoData.forEach((item) => {
            mediaData.append("videos[]", item.video);             // embed code
            mediaData.append("thumbnails[]", item.thumbnail);     // file (image)
        });
    ...
    return (
        <div className='main-content-inner'>
            ...
            <div className="widget-box-2 mb-20">
                <h5 className="title">Add Photos</h5>
                <div
                    className="box-uploadfile text-center"
                    onDrop={handleDrop}
                    onDragOver={handleDragOver}
                >
                    <div className="uploadfile" style={{ padding: "30px 30px" }}>
                        <div
            ...
            <div className="widget-box-2 mb-20">
                <h5 className="title">Add Videos</h5>

                <div className="box-info-property">
                    <div className="box-uploadfile text-center">
                        <div className="uploadfile" style={{ padding: "20px 30px" }}>
                            <a className="btn-upload tf-btn primary" href="#modalVideo" data-bs-toggle="modal">
                                <svg width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="box-img-upload">
                        {videoData.map((item, index) => (
                            <div className="item-upload file-delete" key={index}>
                                <img src={URL.createObjectURL(item.thumbnail)} alt={`video-thumb-${index}`} />
                                <span className="icon icon-trash remove-file" onClick={() => removeVideo(index)}></span>
            ...
            {/* xxxxxxxxxxxxxxxxxxxx Video Modal xxxxxxxxxxxxxxxxxxxx */}
            <div className="modal modal-account fade" id="modalVideo" style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}>
                        ...
                        <div className="flat-account">
                            ...
                            <form className="form-account">
                                ...
                                <div className="title-box">
                                    <h4>Add Video</h4>
                                    <span className="close-modal icon-close2" id='dismisModal' data-bs-dismiss="modal"></span>
                                </div>
                                <div className="box">
                                    <div className="box-info-property">
                                        <fieldset className="box box-fieldset">
                                            <label>
                                                Embed video code:<span className='text-danger'>*</span>
                                            </label>
                                            <textarea
                                                value={currentItem.video}
                                                onChange={(e) => handleCurrentChange(e, "video")}
                                            />

                                        </fieldset>
                                        <fieldset className="box-fieldset">
                                            <label>Video Thumbnail:</label>
                                            <div className="box-floor-img uploadfile d-flex justify-content-between align-items-center">
                                                <div className="btn-upload tf-btn primary">
                                                    ...
                                                    <input
                                                        ...
                                                        className="ip-file"
                                                        accept="image/*"
                                                        onChange={(e) => handleCurrentChange(e, "thumbnail")}
                                                    />
                                                ...
                                                <div className="box-img-upload">
                                                    {currentItem.thumbnail && (
                                                        <div className="item-upload file-delete">
                                                            <img src={URL.createObjectURL(currentItem.thumbnail)} alt="thumbnail-preview" />
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                ...
                                <div className="box box-btn">
                                    <div className="tf-btn primary w-100 cursor-pointer" onClick={handleAdd}>Add</div>
                                </div>
                            </form>
